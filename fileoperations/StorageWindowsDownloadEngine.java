 ...
 * @author TTGETERZI
 */
@DownloadEngine(systemType = "STORAGE-WINDOWS", measType = "PM", operatorName = "VODAFONE")
public class StorageWindowsDownloadEngine extends DefaultTimeBasedEngine {

    public StorageWindowsDownloadEngine(ServerIpList eachIp) {
        super(eachIp);
    }
    ...
    public void onDownlaod(Connection con, ServerIpList connectionInfo, List<RemoteFileObject> fileList) {
        for (RemoteFileObject each : fileList) {
            if (each.getDate().after(getLastDateFromdb())) {
                boolean status = con.downloadFile(DownloadApp.LOCALFILEPATH + each.getFileName(), each.getPath());
                if (status) {
