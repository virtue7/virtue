...
@DownloadEngine(systemType = "STP", measType = "PM", operatorName = "VODAFONE")
public class StpDownloadEngine extends DefaultTimeBasedEngine {

    public StpDownloadEngine(ServerIpList eachIp) {
        super(eachIp);
    }
    ...
    public void onDownlaod(Connection con, ServerIpList connectionInfo, List<RemoteFileObject> fileList) {
        ...
        for (RemoteFileObject each : fileList) {
            ...
            if (reDownloadList.containsKey(each.getFileName())) {
                ...
                if (downloadStatus) {
                    if (localFileName.endsWith(".asn1")) {
                        try {
                            downloadStatus = UnzipOperation.unzipAsn1ToCsv(localFileName);
